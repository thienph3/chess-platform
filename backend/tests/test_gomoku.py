...
class TestGomokuHumanVsHuman:
    ...
    async def test_two_clients_websocket_full_game(self, client: AsyncClient, auth_headers: dict, second_user_headers: dict):
        """REQ: 2 clients connect via WS, play gomoku moves, black wins with 5-in-a-row.

        Simulates the real protocol:
        - Client 1 (white): connects, sends init
        - Client 2 (black): connects, receives state
        - They alternate moves: { "type": "move", "row": X, "col": Y }
        - Analysis service validates → broadcasts new FEN + turn
        - 5 in a row → game_over broadcast to both
        """
        from unittest.mock import AsyncMock, patch
        from starlette.testclient import TestClient
        from app.main import app as fastapi_app
        from app.modules.games.validation_client import ValidationResult

        # Mock validation using local GomokuBoard (avoids HTTP timeout to analysis service)
        board = GomokuBoard()

        async def mock_validate(game_type, fen, move):
            row, col = move.get("row"), move.get("col")
            if not board.is_valid_move(row, col):
                return ValidationResult(valid=False)
            board.push(row, col)
            game_over = board.check_win(row, col)
            result = "black_win" if game_over and board.grid[row][col] == 1 else ("white_win" if game_over else None)
            return ValidationResult(
                valid=True, new_fen=board.to_fen(),
                turn="black" if board.turn == 1 else "white",
                game_over=game_over, result=result, reason="five_in_a_row" if game_over else None,
            )

        async def mock_initial(game_type):
            return {"fen": ";1", "turn": "black", "game_over": False}
        ...
        with patch("app.modules.games.websocket.validate_move", mock_validate), \
             patch("app.modules.games.websocket.get_initial_state", mock_initial):
            sync_client = TestClient(fastapi_app)
            with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws1:
                with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws2:
                    ws1.receive_json()
                    ws2.receive_json()

                    # Init
                    ws1.send_json({"type": "init", "game_type": "gomoku"})
                    state1 = ws1.receive_json()
                    state2 = ws2.receive_json()
                    assert state1["type"] == "state"
                    assert state1["turn"] == "black"

                    # Black wins: row 7, cols 0-4
                    moves = [
                        (7, 0), (8, 0), (7, 1), (8, 1),
                        (7, 2), (8, 2), (7, 3), (8, 3),
                        (7, 4),  # black wins
                    ...
                    for i, (row, col) in enumerate(moves):
                        sender = ws1 if i % 2 == 0 else ws2
                        sender.send_json({"type": "move", "row": row, "col": col})
                        r1 = ws1.receive_json()
                        r2 = ws2.receive_json()
                        assert r1["type"] == "move"
                        assert r2["type"] == "move"
                    ...
                    # Game over
                    over1 = ws1.receive_json()
                    over2 = ws2.receive_json()
                    assert over1["type"] == "game_over"
                    assert over1["result"] == "black_win"
                    assert over1["reason"] == "five_in_a_row"
                    assert over2["type"] == "game_over"

    async def test_two_clients_resign(self, client: AsyncClient, auth_headers: dict, second_user_headers: dict):
        """REQ: Player resign → opponent receives resign message."""
        from starlette.testclient import TestClient
        from app.main import app as fastapi_app
        ...
        with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws1:
            with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws2:
                ws1.receive_json()
                ws2.receive_json()
                ws1.send_json({"type": "resign"})
                msg = ws2.receive_json()
                assert msg == {"type": "resign", "by": "opponent"}

    async def test_two_clients_draw_offer_accept(self, client: AsyncClient, auth_headers: dict, second_user_headers: dict):
        """REQ: Draw offer → opponent accepts → both get game_over draw."""
        from starlette.testclient import TestClient
        from app.main import app as fastapi_app
        ...
        with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws1:
            with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws2:
                ...
                assert ws2.receive_json()["type"] == "draw_offered"
                ws2.send_json({"type": "draw_accept"})
                over1 = ws1.receive_json()
                over2 = ws2.receive_json()
                assert over1 == {"type": "game_over", "result": "draw", "reason": "agreement"}
                assert over2 == {"type": "game_over", "result": "draw", "reason": "agreement"}

    async def test_two_clients_chat(self, client: AsyncClient, auth_headers: dict, second_user_headers: dict):
        """REQ: Chat messages broadcast to both players."""
        from starlette.testclient import TestClient
        from app.main import app as fastapi_app
        ...
        with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws1:
            with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws2:
                ws1.receive_json()
                ws2.receive_json()
                ws1.send_json({"type": "chat", "message": "Chào bạn!"})
                chat1 = ws1.receive_json()
                chat2 = ws2.receive_json()
                assert chat1 == {"type": "chat", "message": "Chào bạn!", "sender": "player"}
                assert chat2 == {"type": "chat", "message": "Chào bạn!", "sender": "player"}

    async def test_invalid_move_only_sender_gets_error(self, client: AsyncClient, auth_headers: dict, second_user_headers: dict):
        """REQ: Invalid move → only sender gets error, opponent not affected."""
        ...
        async def mock_validate(game_type, fen, move):
            ...
            if not board.is_valid_move(row, col):
                return ValidationResult(valid=False)
            board.push(row, col)
            return ValidationResult(valid=True, new_fen=board.to_fen(),
                                    turn="black" if board.turn == 1 else "white")
        ...
        with patch("app.modules.games.websocket.validate_move", mock_validate), \
            ...
            with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws1:
                with sync_client.websocket_connect(f"/ws/game/{room_id}") as ws2:
                    ...
                    # Valid move
                    ws1.send_json({"type": "move", "row": 7, "col": 7})
                    ws1.receive_json()
                    ws2.receive_json()

                    # Same cell again → invalid, only sender gets error
                    ws1.send_json({"type": "move", "row": 7, "col": 7})
                    err = ws1.receive_json()
                    assert err["type"] == "error"
                    assert "không hợp lệ" in err["message"]
